...
waitForDevices(async device => {
    ...
    try {
        ...
        const bitcoinAccount = account.asBitcoinLikeAccount();
        try {
            const result = await bitcoinAccount.broadcastRawTransaction(signedTransaction)
            console.log(">> Result of broadcast: ");
            console.log(result);
        } catch (e) {
...
async function createTransaction(wallet, account) {
    const ADDRESS_TO_SEND = '2MvuUMAG1NFQmmM69Writ6zTsYCnQHFG9BF'

    const bitcoinLikeAccount = account.asBitcoinLikeAccount()
    const walletCurrency = wallet.getCurrency()
    const amount = createAmount(walletCurrency, 1000)
