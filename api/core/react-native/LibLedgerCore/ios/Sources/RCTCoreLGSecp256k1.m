...
-(instancetype)init
{
    ...
    if(self)
    {
        self.objcImpl = [LGSecp256k1 createInstance];
    }
    return self;
}
 ...
 */
RCT_REMAP_METHOD(computePubKey,computePubKey:(NSString *)privKey
                                    compress:(BOOL)compress withResolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject) {
    
    NSData *privKeyData = [RCTConvert NSData:privKey];
    //NSData *privKeyData = [[NSData alloc] initWithBytes:[privKeyDict[@"privKey"] bytes] length:sizeof([privKeyDict[@"privKey"] bytes])];
    NSData *pubKey = [self.objcImpl computePubKey:privKeyData compress:compress];
    //NSString *pubKeyString = [RCTConvert NSString:pubKey];
    NSString *pubKeyString = nil;
    NSStringEncoding pubKeyEndoding = [NSString stringEncodingForData:pubKey encodingOptions:nil convertedString:&pubKeyString usedLossyConversion:nil];
    //NSString *pubKeyString
    id result = @{@"result" : pubKeyString};
    if(result)
    {
        resolve(result);
    }
