...
void NJSAccountListCallback::onCallback(const std::experimental::optional<std::vector<std::shared_ptr<Account>>> & result, const std::experimental::optional<Error> & error)
{
    ...
    {
        Nan::ThrowError("NJSAccountListCallback::onCallback fail to retrieve node implementation");
    }
    ...
    {
        Nan::ThrowError("NJSAccountListCallback::onCallback call failed");
    }
}

NAN_METHOD(NJSAccountListCallback::addRef) {

    NJSAccountListCallback *obj = Nan::ObjectWrap::Unwrap<NJSAccountListCallback>(info.This());
    obj->Ref();
}

NAN_METHOD(NJSAccountListCallback::removeRef) {
...
NAN_METHOD(NJSAccountListCallback::New) {
    ...
    NJSAccountListCallback *node_instance = nullptr;
    if(info[0]->IsObject())
    {
        node_instance = new NJSAccountListCallback(info[0]->ToObject());
    }
    else
    {
        return Nan::ThrowError("NJSAccountListCallback::New requires an implementation from node");
    }
