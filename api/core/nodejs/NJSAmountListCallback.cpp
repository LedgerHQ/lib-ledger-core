...
void NJSAmountListCallback::onCallback(const std::experimental::optional<std::vector<std::shared_ptr<Amount>>> & result, const std::experimental::optional<Error> & error)
{
    ...
    if(result)
    {
        ...
        {
            auto arg_0_tmp_elem = NJSAmount::wrap(arg_0_optional[arg_0_tmp_id]);
...
NAN_METHOD(NJSAmountListCallback::New) {
    ...
    auto node_instance = std::make_shared<NJSAmountListCallback>(resolver);
    djinni::js::ObjectWrapper<NJSAmountListCallback>::Wrap(node_instance, info.This());
