...
{
    BLAKE2B_PARAM blakeParams;
    blakeParams.digest_length = outlen;
    blakeParams.key_length = 0;
    blakeParams.fanout = 1;
    blakeParams.depth = 1;
    store32(blakeParams.leaf_length, 0);
    store64(blakeParams.node_offset, 0);
    blakeParams.node_depth = 0;
    blakeParams.inner_length = 0;
    memset(blakeParams.reserved, 0, sizeof(blakeParams.reserved));
    memset(blakeParams.salt, 0, sizeof(blakeParams.salt));
    memset(blakeParams.personal, 0, sizeof(blakeParams.personal));
    BLAKE2b_Init(c, &blakeParams);
    return 1;
}
