 ...
 */
void MD5_Init_Local(MD5_CTX *ctx) {
...
void MD5_Update_Local(MD5_CTX *ctx, const unsigned char *buf, size_t len) {
  uint32_t t;
...
void MD5_Final_Local(unsigned char digest[16], MD5_CTX *ctx) {
  unsigned count;
  unsigned char *p;
  uint32_t *a;
...
char *cs_md5(char buf[33], ...) {
  ...
  while ((p = va_arg(ap, const unsigned char *) ) != NULL) {
    ...
    MD5_Update_Local(&ctx, p, len);
  }
  va_end(ap);

  MD5_Final_Local(hash, &ctx);
