        ...
        namespace tests {
            ...
            public:
                AccountSyncTest()
                    ...
                    , fakeStableDBBackend(new db::BlockchainDBInMemory())
                    , fakeUnstableDBBackend(new db::BlockchainDBInMemory())
                    , fakeStableDB(context, fakeStableDBBackend)
                    , fakeUnstableDB(context, fakeUnstableDBBackend){
            ...
            private:
                void linkMockDbToFake(std::shared_ptr<NiceMock<BlocksDBMock>>& mock, common::PersistentBlockchainDatabase<BitcoinLikeNetwork>& fake) {
                    ON_CALL(*mock, addBlock(_)).WillByDefault(Invoke(&fake, &common::PersistentBlockchainDatabase<BitcoinLikeNetwork>::addBlock));
                    ON_CALL(*mock, removeBlocks(_, _)).WillByDefault(Invoke(&fake, &common::PersistentBlockchainDatabase<BitcoinLikeNetwork>::removeBlocks));
                    ON_CALL(*mock, removeBlocksUpTo(_)).WillByDefault(Invoke(&fake, &common::PersistentBlockchainDatabase<BitcoinLikeNetwork>::removeBlocksUpTo));
                    ON_CALL(*mock, CleanAll()).WillByDefault(Invoke(&fake, &common::PersistentBlockchainDatabase<BitcoinLikeNetwork>::CleanAll));
                    ON_CALL(*mock, getBlocks(_, _)).WillByDefault(Invoke(&fake, &common::PersistentBlockchainDatabase<BitcoinLikeNetwork>::getBlocks));
                    ON_CALL(*mock, getBlock(_)).WillByDefault(Invoke(&fake, &common::PersistentBlockchainDatabase<BitcoinLikeNetwork>::getBlock));
                    ON_CALL(*mock, getLastBlockHeader()).WillByDefault(Invoke(&fake, &common::PersistentBlockchainDatabase<BitcoinLikeNetwork>::getLastBlockHeader));
                }
            public:
                std::shared_ptr<SimpleExecutionContext> context;
                // TODO: redo 4 next lines with InMemoryBlockchainDatabase when it will be ready
                std::shared_ptr<db::BlockchainDBInMemory> fakeStableDBBackend;
                std::shared_ptr<db::BlockchainDBInMemory> fakeUnstableDBBackend;
                common::PersistentBlockchainDatabase<BitcoinLikeNetwork> fakeStableDB;
                common::PersistentBlockchainDatabase<BitcoinLikeNetwork> fakeUnstableDB;
