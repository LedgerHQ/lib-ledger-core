...
struct TezosMakeTransaction :
    ...
    void SetUpConfig() override {
        ...
        testData.inflate_coin = ledger::testing::xtz::inflate;
    }
};
...
TEST_F(TezosMakeTransaction, CreateTx) {
    ...
    builder->setStorageLimit(std::make_shared<BigInt>(BigInt::fromString("1000")));
    ...
    //Delete account
    auto code = wait(walletStore->eraseDataSince(formatedDate));
    ...
    txBuilder->setStorageLimit(std::make_shared<BigInt>(BigInt::fromString("1000")));
...
TEST_F(TezosMakeTransaction, ParseUnsignedRawTransaction) {
    ...
    auto tx = api::TezosLikeTransactionBuilder::parseRawUnsignedTransaction(currencies::tezos(), txBytes);
...
TEST_F(TezosMakeTransaction, ParseSignedRawDelegationTransaction) {
    ...
    auto txBytes = hex::toByteArray(strTx);
    auto tx = api::TezosLikeTransactionBuilder::parseRawSignedTransaction(currencies::tezos(), txBytes);
